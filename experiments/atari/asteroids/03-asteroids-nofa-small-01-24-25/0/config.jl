...
env_args = (
    name="AsteroidsDeterministic-v4",
    n_envs=1,
    n_steps=2500,
    n_stack=4,
    record_prefix="",
...
geno_creator = Creator(Delta, Creator(JevoChain, (rng, gene_counter, [
    (Jevo.Conv, (kernel=(8,8), channels=3*env_args.n_stack=>4, stride=(4,4), σ=relu, )),
    (Jevo.Conv, (kernel=(4,4), channels=4=>8, stride=(2,2), σ=relu, )),
    (Jevo.Conv, (kernel=(3,3), channels=8=>8, stride=(1,1), σ=relu, )),
    Flux.flatten,
    (Jevo.Dense, (dims=(392,32),σ=relu, )),
    (Jevo.Dense, (dims=(32,14),σ=identity))
])))
...
state = if isfile(checkpointname)
        ...
        State("", rng, [pop_creator, env_creator], 
            [ InitializeAllPopulations(),
                    CreateMissingWorkers(n_workers, slurm=true, c=2, n_gpus=1),
                    InitializePhylogeny(),
